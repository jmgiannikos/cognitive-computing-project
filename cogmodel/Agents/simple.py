...
class simple(object):
    ...
    def perform_action(self):
        ...
        if len(self._possible_actions)<1:
            ...
            #print(self._possible_actions)
            action=self._possible_actions[np.random.randint(0,high=len(self._possible_actions))]
            #print(action)
    ...
    def turn_left(self):
        ...
        self.rotation_matrix = np.matmul(np.array([[0,-1],[1,0]]),self.rotation_matrix)


    def turn_right(self):
        self.env.perform_action(action=ACTION_MAPPING["TURN RIGHT"],agent=self)
        self.rotation_matrix = np.matmul(np.array([[0,1],[-1,0]]),self.rotation_matrix)
    
    def turn_back(self):
        self.env.perform_action(action=ACTION_MAPPING["TURN RIGHT"],agent=self)
        self.env.perform_action(action=ACTION_MAPPING["TURN RIGHT"],agent=self)
        self.rotation_matrix = np.matmul(np.array([[-1,0],[0,-1]]),self.rotation_matrix)

    def go_left(self):
        self.turn_left()
        self.predict_next_actions()
        self.env.perform_action(action=tuple(np.matmul(self.rotation_matrix,NORTH)),agent=self)
