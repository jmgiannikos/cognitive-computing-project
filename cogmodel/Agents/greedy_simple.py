...
class greedy(object):
    ...
    def _choose_action(self):
        ...
        if not acted:
            ...
            self.last_pos = self.env.agent_pos
            self.env.perform_action((best_option[0]-self.env.agent_pos[0],best_option[1]-self.env.agent_pos[1]), self)
            if self.env.agent_pos in self.visited.keys():
                self.visited[self.env.agent_pos] += 1
            else:
                self.visited[self.env.agent_pos] = 1
    ...
    def tile_score(self, tile):
        ...
        distance_score = math.sqrt((tile[0]-self.env.target[0])**2)/self.env.size[0] + math.sqrt((tile[1]-self.env.target[1])**2)/self.env.size[1]
